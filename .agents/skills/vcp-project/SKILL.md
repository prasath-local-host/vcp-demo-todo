---
name: vcp-project
description: Apply the VCP policy and infrastructure profile when working in the vcp-demo-todo repository, including code, dependencies, architecture, and deployment changes.
---

# VCP project adapter for Codex

At the start of each session and before architecture or dependency changes, read the repository-root AGENTS.md and vcp.project.json in full. Retain AGENTS.md as the canonical always-loaded entry point when this repository is the Codex working directory. Do not modify the canonical policy to accommodate this adapter.

For each new session, explicitly invoke $vcp-project if it was not loaded. When working from a parent workspace or a client without project skill discovery, attach this SKILL.md plus the root AGENTS.md and vcp.project.json. Automatic loading has not been verified in a fresh Codex session.

Codex does not provide the Claude @-import mechanism for skill contents. The complete canonical policy and profile are copied below. Source paths: /AGENTS.md and /vcp.project.json; policy version: 1.1.0. Compare these copies to their sources and refresh them when either source changes. The source files govern; these copies grant no new approval. Contact the VCP team before unsupported implementation, and continue independent work within scope.

## Canonical policy copy
...

{
  ...
  "ui": "react",
  "uiMajor": 18,
  "styling": "tailwindcss3",
  "database": "postgresql17",
  "databaseDriver": "pg",
  "architecture": "single-application-server",
  "packageManager": "npm",
  "deploymentOwner": "VCP",
  "provisioningStatus": "requires-vcp-setup",
  "externalServices": [],
  "additionalInfrastructure": [],
  "support": "Contact the VCP team through your established company support channel"
}
